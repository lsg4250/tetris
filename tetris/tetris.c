...
int block_move() { // 수정 : 함수 이름이 block_drop인데, 코드 보면 블록 이동을 전체적으로 제어하길래 이름을 move로 바꿨습니다.
	...
	DWORD last = GetTickCount(); // 추가 : 시스템 부팅 후 시간을 기록하는 코드입니다. 이후에 블록이 떨어지는 시점을 더 잘 관리하려고 넣었습니다.
	...
	while (1) {
		...
		DWORD now = GetTickCount(); //GetTickCount로 현재 시간을 받아옵니다.
...
// 추가 : 블록이 이상한 범위로 나가거나 놓을 수 없는 범위에 도달하는 경우를 막기 위해 만든 함수입니다. 충돌검사 코드입니다.
